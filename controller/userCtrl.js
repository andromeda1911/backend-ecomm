...
const removeFromWishlist = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  console.log("removewishlist----req----", req.body);
  try {
    let user = await User.updateOne(
      { _id: _id },
      { $pull: { wishlist: { $in: req.body.wishlistId } } },
      { new: true }
    );
    res.json(user);
  } catch (error) {
...
const getMyOrders = asyncHandler(async(req, res) => {
  const { _id } = req.user;
  try {
    const orders = await Order.find({user:_id}).populate("user").populate("orderItems.product").populate("orderItems.color").populate("orderItems.size");
    res.json({
      orders
    })
  } catch (error) {
